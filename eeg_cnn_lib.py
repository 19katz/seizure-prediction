...
import lasagne
from lasagne.layers import Conv2DLayer, MaxPool2DLayer, InputLayer, DenseLayer
from lasagne.layers import ConcatLayer, ReshapeLayer, get_output_shape
from lasagne.layers import LSTMLayer, SliceLayer, FlattenLayer
...
def gen_images(locs, features, n_gridpoints, normalize=False,
    ...
    :return:            Tensor of size [samples, colors, W, H] containing generated
                        images.
    """
    print('locs: {}, n_gridpoints: {}'.format(locs, n_gridpoints))
    feat_array_temp = []
    ...
    for c in range(n_colors):
        ...
        # print('Feature temp shape:{}, featues:{}'.format(len(feat_array_temp.shape), feat_array_temp))
    if augment:
        if pca:
            for c in range(n_colors):
...
def build_cnn(input_var=None, w_init=None, n_layers=(2, 2), n_filters_first=32, imsize=16, n_colors=6):
    """
...
def build_convpool_lstm(input_vars, nb_classes, grad_clip=110, imsize=16, n_colors=6, n_timewin=10):
    """
    ...
    convnets = []
    w_init = None
    # Build n_timewin parallel CNNs with shared weights
...
def train(train_val_test_data, model_type, batch_size=10, num_epochs=5, n_colors=6):
    """
    ...
    (X_train, y_train, X_val, y_val, X_test, y_test) = train_val_test_data
    ones = [i for i in range(len(y_train)) if y_train[i] == 1]
    # X_preictals = X_train[ones]
    # y_preictals = y_train[ones]

    print('In train(): train data shape:', X_train.shape, ', num of preictals:', ones)
    ...
    if model_type == 'lstm':
        network = build_convpool_lstm(input_var, num_classes, 100, n_colors=n_colors)
    elif model_type == 'cnn':
        input_var = T.tensor4('inputs')
        network, _ = build_cnn(input_var, n_colors=n_colors)
        ...
        raise ValueError("Model not supported ['lstm', 'cnn']")
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_val, y_val, batch_size, shuffle=False):
            inputs, targets = batch
            err, acc, res = val_fn(inputs, targets)
            val_err += err
            val_acc += acc
            val_batches += 1
